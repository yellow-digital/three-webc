...
export default function (viewport) {
  ...
  // var gui = new dat.GUI();
  // gui.add(state, "radius").min(85).max(1000).step(5);
  // gui.add(state, "speed").min(0.5).max(10).step(0.01);
  // gui
  //   .add(state, "elevation")
  //   .min(0.0)
  //   .max(Math.PI / 2.0)
  ...
  //   .listen()
  //   .onChange(function (value) {
  //     backgroundMaterial.uniforms.fogFade.value = fogFade;
  //   });
  // gui.close();
  ...
  function draw(dT, { time }) {
    // console.log(time)
    const SPEED = 0.8
    grassMaterial.uniforms.time.value = time / 1000 * SPEED;

    // renderer.clear();
    // renderer.render(scene, camera);
    // viewport.requestAnimationFrame(draw)
  }
